...
public abstract class ItemViewHolder<D> extends RecyclerView.ViewHolder implements NotifyCallback<D>, ViewOperation, LayoutItem {
     ...
     * <p>你并不需要将模型中的所有字段进行比较，只需要比较需要展示在界面上的字段就可以了。如果你实现了该方法那么就必须要实现
     * {@link #onBindPartData(int, Object, Bundle)} 方法，否则是没有意义的。
     *
     * @param oldItemData 旧的数据模型。
     * @param newItemDate 新的数据模型。
     * @param bundle      比较两个对象不同的部分，将两个对象不同的部分以键值对的形式存入该参数中。这个参数你会在
     *                    {@link #onBindPartData(int, Object, Bundle)} 方法中获得。
     * @see #onBindPartData(int, Object, Bundle)
     */
    @Override
    ...
    /**
     * 绑定数据的时候调用。与 {@link #onBindData(int, Object)} 方法不同的是，这个方法可以用来局部绑定数据。
     * 就是说当一个条目中的数据模型发生变化以后但不是展示在UI上的每个字段都发生了变化的情况下就可以通过该方法只针对部分改动过的字段进行数据绑定。
     * <p>但是该方法也不是必然就会被执行的，而是在 {@link #getChangePayload(Object, Object, Bundle)} 方法中获得了不同之处后才会被执行。
     * <p>该方法的默认实现是调用 {@link #onBindData(int, Object)} 方法，如果你希望通过局部刷新来提高效率则重写该方法。
     *
     * @param position 当前的Item索引。
     * @param d        当前索引对应的数据对象。
     * @param payloads 数据模型被改变的内容。是 {@link #getChangePayload(Object, Object, Bundle)}
     *                 方法中的第三个参数。该参数永远不可能为null。否则改方法不会被调用，而是调用 {@link #onBindData(int, Object)} 方法。
     * @see #getChangePayload(Object, Object, Bundle)
     */
    public void onBindPartData(int position, D d, Bundle payloads) {
